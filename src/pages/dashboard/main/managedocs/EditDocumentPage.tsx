...
const UploadDocumentPage: React.FC = () => {
  ...
  // Use string for input field to allow empty state, but treat as number for logic
  ...
  // --- REFINED: Handle Price Change to allow empty input ---
  ...
  // --- REFINED: Handle Submit with explicit data validation ---
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Convert price state to a number for validation and submission
    const numericPrice = Number(price);

    if (isNaN(numericPrice) || numericPrice < 0) {
      setFormError('Price must be a valid number and cannot be negative.');
      return;
    }

    try {
      // Ensure the payload has a valid number for the price
      await createDocument({
        title,
        description,
        genre,
        price: numericPrice, // Send the validated numeric price
        file: selectedFile,
        isFeatured,
      }).unwrap();
      ...
      // The API error will be displayed automatically by the `apiError` state
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-0 py-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 shadow-lg rounded-lg space-y-8">
            ...
            {!selectedFile ? (
              ...
              <div className="flex items-center justify-between p-4 bg-slate-100 border border-slate-200 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-gray-800 truncate">{selectedFile.name}</span>
                </div>
                <button type="button" onClick={() => setSelectedFile(null)} className="p-1 text-gray-500 hover:text-red-600 rounded-full">
                  <X size={20} />
                </button>
              </div>
            )}
            ...
            <div>
              ...
              {isFree && (
                <div className="flex items-center gap-2 text-sm text-green-700 bg-green-50 p-2 mt-2 rounded-md">
                  <Info size={16} />
                  <span>This document will be marked as free.</span>
                </div>
              )}
            </div>
          ...
          <div className="border-t pt-6 flex justify-end space-x-4">
            ...
            <Button type="submit" isLoading={isLoading}>Upload Document</Button>
          </div>
        </form>
      </div>
    </div>
