...
import {
  ...
  Search,
  UploadCloud,
  BookOpen // <-- NEW: Imported icon for Genre
} from 'lucide-react';
...
const AdminDocumentsPage: React.FC = () => {
  ...
  // --- RTK QUERY HOOKS ---
  const { data: response, error, isLoading } = useFetchDocumentsQuery({
    page,
    limit: 15,
    searchTerm: searchTerm || undefined, // Pass search term to the API
  });
      ...
      <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {documents.length > 0 ? (
                  ...
                  // --- MODIFIED: Colspan updated for the new column ---
                  <tr><td colSpan={6} className="text-center py-10 text-gray-500">No documents found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="block md:hidden space-y-4">
          {documents.length > 0 ? (
            ...
            <div className="text-center py-10 text-gray-500 bg-white rounded-lg shadow-md">No documents found.</div>
          )}
          {totalPages > 1 && <PaginationControls />}
        </div>
